...
Some rotary encoders appear to generate 2 steps per click (issue #18).
A code review shows this "double counting" is definitely generated by the
rotary encoder device.

The easiest way to handle this is add a divide by 2.
...
Mapping of the raw "signal" is not a function of the library as it depends
on the functionality of the project.
Furthermore this mapping can differ per rotary encoder making this library
more complex than needed.
Therefore a dedicated "divide by 2" option is not implemented.

See e.g. - https://github.com/RobTillaart/multiMap for non linear mapping.
...
- **int32_t getValue(uint8_r re)** returns the rotary encoder counter.
...
- add **void reset(uint8_t re)** per channel
